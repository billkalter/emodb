...
public class AstyanaxTableDAO implements TableDAO, MaintenanceDAO, StashTableDAO, Managed {
    ...
    public Iterator<UnpublishedDatabusEvent> listUnpublishedDatabusEvents(Date fromInclusive, Date toExclusive) {
        ...
        LocalDate fromInclusiveUTC = fromInclusive.toInstant().atZone(ZoneOffset.UTC).toLocalDate();
        LocalDate toInclusiveDateUTC = toExclusive.toInstant().atZone(ZoneOffset.UTC).toLocalDate();
        if (toExclusive.toInstant().atZone(ZoneOffset.UTC).get(ChronoField.MILLI_OF_DAY) == 0) {
            toInclusiveDateUTC = toInclusiveDateUTC.minusDays(1);
        }

        // adding 1 to the days to make it inclusive.
        final int inclusiveNumberOfDays = Period.between(fromInclusiveUTC, toInclusiveDateUTC).getDays() + 1;

...
public class SecurityModule extends PrivateModule {
    ...
    AuthIdentityManager<ApiKey> provideAuthIdentityManagerWithDefaults(
        ...
        reservedIdentities.put(replicationKey,
                createReservedApiKey(REPLICATION_INTERNAL_ID, "replication", ImmutableSet.of(DefaultRoles.replication.toString())));
        reservedIdentities.put(adminKey,
                createReservedApiKey(ADMIN_INTERNAL_ID, "admin", ImmutableSet.of(DefaultRoles.admin.toString())));

        if (anonymousKey.isPresent()) {
            reservedIdentities.put(anonymousKey.get(),
                    createReservedApiKey(ANONYMOUS_INTERNAL_ID, "anonymous", ImmutableSet.of(DefaultRoles.anonymous.toString())));
        }

        return new DeferringAuthIdentityManager<>(daoManager, reservedIdentities.build());
    }

    private ApiKey createReservedApiKey(String internalId, String description, Set<String> roles) {
        ApiKey apiKey = new ApiKey(internalId, roles);
